...
describe('InspectorController', () => {
  ...
  describe('loadFromRequest', () => {
    it('sets the origin display and path from the captured request URL', () => {
      const { ctrl } = makeController();
      ctrl.loadFromRequest({ url: 'https://api.example.com/v1/items?page=2&limit=10' });

      expect(document.getElementById('origin-display').value).toBe('https://api.example.com');
      expect(document.getElementById('path-input').value).toBe('/v1/items');
    });

    it('populates params from the captured URL query string as custom source', () => {
      const { ctrl } = makeController();
      ctrl.loadFromRequest({ url: 'https://api.example.com/search?q=foo&lang=en' });

      const params = ctrl.getParams();
      expect(params).toContainEqual(expect.objectContaining({ key: 'q',    value: 'foo', enabled: true }));
      expect(params).toContainEqual(expect.objectContaining({ key: 'lang', value: 'en',  enabled: true }));
    });

    it('replaces existing default and custom params with the captured ones', async () => {
      chrome.tabs.query.mockResolvedValueOnce([{ id: 1, url: 'https://example.com/?original=1' }]);
      const { ctrl } = makeController();
      await ctrl.init();

      ctrl.loadFromRequest({ url: 'https://api.example.com/v2/data?new=2' });

      const params = ctrl.getParams();
      expect(params.map(p => p.key)).not.toContain('original');
      expect(params.map(p => p.key)).toContain('new');
    });

    it('preserves existing profile params untouched', async () => {
      chrome.tabs.query.mockResolvedValueOnce([{ id: 1, url: 'https://example.com/' }]);
      const { ctrl } = makeController({
        readProfiles:       vi.fn().mockResolvedValue({ Dev: { params: [{ enabled: true, key: 'env', value: 'dev' }], headers: [] } }),
        loadEnabledProfile: vi.fn().mockResolvedValue('Dev'),
      });
      await ctrl.init();

      ctrl.loadFromRequest({ url: 'https://api.example.com/v1/items?page=1' });

      const profileParams = ctrl._params.filter(p => p.source === 'Dev');
      expect(profileParams).toHaveLength(1);
      expect(profileParams[0].key).toBe('env');
    });

    it('handles a URL with no query string (adds no params)', () => {
      const { ctrl } = makeController();
      ctrl.loadFromRequest({ url: 'https://api.example.com/health' });

      expect(ctrl.getParams()).toHaveLength(0);
      expect(document.getElementById('path-input').value).toBe('/health');
    });

    it('silently ignores an invalid URL and leaves state unchanged', () => {
      const { ctrl } = makeController();
      const before = document.getElementById('origin-display').value;
      ctrl.loadFromRequest({ url: 'not-a-valid-url' });
      expect(document.getElementById('origin-display').value).toBe(before);
    });
  });
});
